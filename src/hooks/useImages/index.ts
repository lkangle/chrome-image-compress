...
const someScale = (images: CdnImage[]) => {
    const rs = images.map((img) => +(img.width / img.height).toFixed(2));
...
function useImages(showDrawer: VoidFunction): IResult {
    ...
    const imageList = useMemo(() => {
        const uniqImages = uniqBy(concat(uploadImages, dbImages), 'url').filter(
            (it) => !!it.url
        );
        ...
        // 合并后的图片列表
        const unionImages = groupList.reduce(
            (arr, group: { key: string; images: CdnImage[] }) => {
